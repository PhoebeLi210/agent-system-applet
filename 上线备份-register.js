...
Page({
  ...
  onLoad(options) {
    ...
    if (!agentCode) {
      agentCode = 'T6FAPXF3';
      console.log('测试用代理商标识码:', agentCode);
    }
  ...
  loadFromStorage() {
    const app = getApp();
    const apiBaseUrl = app.globalData.apiBaseUrl || 'https://agent.lakala.space';
  ...
  fillFormFromOCR(type, ocrData) {
    ...
    if (type === 'idCardFront') {
      ...
      if (ocrData.startDate) {
        let startDateStr = ocrData.startDate;
        if (/^(\d{8})$/.test(startDateStr)) {
          startDateStr = startDateStr.replace(/(\d{4})(\d{2})(\d{2})/, '$1.$2.$3');
        }
