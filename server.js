...
let SQL = null;
let db = null;
const DEFAULT_AGENT_PASSWORD = process.env.DEFAULT_AGENT_PASSWORD || '';
...
app.post('/api/save-merchant', (req, res) => {
    try {
        ...
        const agentCode = req.query.agent || req.body.agent || merchantInfo.agentCode;
...
app.post('/api/agents', (req, res) => {
    try {
        ...
        // 为代理商创建用户，优先使用请求中指定密码，否则使用默认密码或生成安全密码
        const initialPassword = req.body.password || DEFAULT_AGENT_PASSWORD || generateSecurePassword();
        const hashedPassword = bcrypt.hashSync(initialPassword, 10);
        dbRun(`
        INSERT INTO users (username, password, agent_id, role)
        VALUES (?, ?, ?, ?)
        `, [agentCode, hashedPassword, result.lastID, 'agent']);
        
        res.json({ success: true, data: { id: result.lastID, agentCode, agentName, sceneId: finalSceneId, password: initialPassword } });
    } catch (error) {
...
app.post('/api/agents/:id/reset-password', (req, res) => {
    try {
        ...
        // 如果未指定新密码，则使用默认密码或生成安全密码
        const resetPassword = req.body.newPassword || DEFAULT_AGENT_PASSWORD || generateSecurePassword();
        const hashedPassword = bcrypt.hashSync(resetPassword, 10);
        ...
        res.json({ success: true, message: '密码重置成功', password: resetPassword });
    } catch (error) {
